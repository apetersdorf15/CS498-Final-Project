...
void ROSThread::ros_initialize(ros::NodeHandle &n)
{
  ...
  velodyne_combined_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/velodyne_points", 1000);
...
void ROSThread::VelodyneLeftThread()
{
  ...
  int left_current_file_index = 0;
  int left_previous_file_index = 0;
  int right_current_file_index = 0;
  int right_previous_file_index = 0;
  string target = "camera_init";

  Eigen::Matrix4f left_transform;
  left_transform(0,0) = -0.516377;
  left_transform(0,1) = -0.702254;
  left_transform(0,2) = -0.490096;
  left_transform(0,3) = -0.334623 - 0.07;
  left_transform(1,0) =  0.491997;
  left_transform(1,1) = -0.711704;
  left_transform(1,2) =  0.501414;
  left_transform(1,3) =  0.431973;
  left_transform(2,0) = -0.700923;
  left_transform(2,1) =  0.0177927;
  left_transform(2,2) =  0.713015;
  left_transform(2,3) =  1.94043 + 1.7;
  left_transform(3,0) =         0;
  left_transform(3,1) =         0;
  left_transform(3,2) =         0;
  left_transform(3,3) =         1;

  Eigen::Matrix4f right_transform;
  right_transform(0,0) = -0.514521;
  right_transform(0,1) = 0.701075;
  right_transform(0,2) = -0.493723;
  right_transform(0,3) = -0.333596 - 0.07;
  right_transform(1,0) =  -0.492472;
  right_transform(1,1) = -0.712956;
  right_transform(1,2) =  -0.499164;
  right_transform(1,3) =  -0.373928;
  right_transform(2,0) = -0.701954;
  right_transform(2,1) =  -0.0136853;
  right_transform(2,2) =  0.712091;
  right_transform(2,3) =  1.94377 + 1.7;
  right_transform(3,0) =         0;
  right_transform(3,1) =         0;
  right_transform(3,2) =         0;
  right_transform(3,3) =         1;
  ...
  while(1){
    ...
    while(!velodyne_left_thread_.data_queue_.empty() || !velodyne_right_thread_.data_queue_.empty()){
      sensor_msgs::PointCloud2 left_cloud;
      sensor_msgs::PointCloud2 right_cloud;
      sensor_msgs::PointCloud2 base_left_cloud;
      sensor_msgs::PointCloud2 base_right_cloud;
      if(!velodyne_left_thread_.data_queue_.empty()){
        ...
        if(to_string(data) + ".bin" == velodyne_left_next_.first){
          ...
          left_cloud = velodyne_left_next_.second;
  ...
  //        cout << "Re-load left velodyne from path" << endl;
          ...
          if(find(next(velodyne_left_file_list_.begin(),max(0,left_previous_file_index-search_bound_)),velodyne_left_file_list_.end(),to_string(data)+".bin") != velodyne_left_file_list_.end()){
              ifstream file;
              ...
              left_cloud = publish_cloud;
              velodyne_left_pub_.publish(publish_cloud);

          }
          left_previous_file_index = 0;
        }
        ...
        cloud.clear();
        sensor_msgs::PointCloud2 publish_cloud;
        left_current_file_index = find(next(velodyne_left_file_list_.begin(),max(0,left_previous_file_index-search_bound_)),velodyne_left_file_list_.end(),to_string(data)+".bin") - velodyne_left_file_list_.begin();
        if(find(next(velodyne_left_file_list_.begin(),max(0,left_previous_file_index-search_bound_)),velodyne_left_file_list_.end(),velodyne_left_file_list_[left_current_file_index+1]) != velodyne_left_file_list_.end()){
            string next_file_name = data_folder_path_ + "/sensor_data/VLP_left" +"/"+ velodyne_left_file_list_[left_current_file_index+1];
            ...
            file.close();
            pcl::toROSMsg(cloud, publish_cloud);
            velodyne_left_next_ = make_pair(velodyne_left_file_list_[left_current_file_index+1], publish_cloud);
        }
        left_previous_file_index = left_current_file_index;
    }

      if(!velodyne_right_thread_.data_queue_.empty()){
      ...
      if(to_string(data) + ".bin" == velodyne_right_next_.first){
        ...
        right_cloud = velodyne_right_next_.second;
...
//        cout << "Re-load right velodyne from path" << endl;
        ...
        if(find(next(velodyne_right_file_list_.begin(),max(0,right_previous_file_index-search_bound_)),velodyne_right_file_list_.end(),to_string(data)+".bin") != velodyne_right_file_list_.end()){
            ifstream file;
            file.open(current_file_name, ios::in|ios::binary);
            while(!file.eof()){
                pcl::PointXYZI point;
            ...
            right_cloud = publish_cloud;
            velodyne_right_pub_.publish(publish_cloud);

        }
        right_previous_file_index = 0;
      }
      ...
      cloud.clear();
      sensor_msgs::PointCloud2 publish_cloud;
      right_current_file_index = find(next(velodyne_right_file_list_.begin(),max(0,right_previous_file_index-search_bound_)),velodyne_right_file_list_.end(),to_string(data)+".bin") - velodyne_right_file_list_.begin();
      if(find(next(velodyne_right_file_list_.begin(),max(0,right_previous_file_index-search_bound_)),velodyne_right_file_list_.end(),velodyne_right_file_list_[right_current_file_index+1]) != velodyne_right_file_list_.end()){
          string next_file_name = data_folder_path_ + "/sensor_data/VLP_right" +"/"+ velodyne_right_file_list_[right_current_file_index+1];
          ...
          file.close();
          pcl::toROSMsg(cloud, publish_cloud);
          velodyne_right_next_ = make_pair(velodyne_right_file_list_[right_current_file_index+1], publish_cloud);
      }

      right_previous_file_index = right_current_file_index;
    }
      
      if(left_cloud.is_dense && right_cloud.is_dense){
        pcl_ros::transformPointCloud(left_transform, left_cloud, base_left_cloud);
        pcl_ros::transformPointCloud(right_transform, right_cloud, base_right_cloud);

        // Merge metadata
        base_right_cloud.width += base_left_cloud.width;

        // Re-size the merged data array to make space for the new points
        uint64_t OriginalSize = base_right_cloud.data.size();
        base_right_cloud.data.resize(base_right_cloud.data.size() + base_left_cloud.data.size());

        // Copy the new points from Cloud1 into the second half of the MergedCloud array
        std::copy(
          base_left_cloud.data.begin(),
          base_left_cloud.data.end(),
          base_right_cloud.data.begin() + OriginalSize);
        //combined_lidar = base_left_cloud + base_right_cloud;
        base_right_cloud.header.frame_id = "base_link";
        velodyne_combined_pub_.publish(base_right_cloud);
    }
    }
    if(velodyne_left_thread_.active_ == false) return;
  }
